<?php
	...
	if (isset($_POST['reg_user'])) {
		...
		$user_check_query = "SELECT * FROM admin_user WHERE username='$username' OR email= '$email' LIMIT 1";
		...
		if (count($errors) == 0) {
			...
			$query = "INSERT INTO admin_user (username, email, password)
		...
		if (in_array($imageActualExt, $allowed))	{
			if ($imageError === 0){
				if ($imageSize < 3000000){
					...
					$insert_sql = "INSERT INTO gallery (title, description, image_name) VALUES('$title','$description','$imageNameNew')";
					mysqli_query($db, $insert_sql) or die("database error: ". mysqli_error($db));
					$_SESSION['thats_ok'] = "Image successfully uploaded";
       				$_SESSION['thats_okk'] = "success";
